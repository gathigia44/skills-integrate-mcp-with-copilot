...
def get_activities(
    ...
    if search:
        ...
        filtered = filter(lambda item: search_lower in item[0].lower() or search_lower in item[1].get("description", "").lower(), filtered)
    filtered = list(filtered)
    # Sort
    if sort == "name":
        filtered.sort(key=lambda item: item[0].lower())
    elif sort == "time":
        filtered.sort(key=lambda item: item[1].get("schedule", ""))
    # Return as dict
